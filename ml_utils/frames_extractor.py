...
parser.add_argument('--video_path', type=str, required=True,
                    help='Path of the video to extract frames from')
parser.add_argument('--frame_name', type=str,
                    help='Optional frame name.')
parser.add_argument('--frame_format', type=str,
                    help='extracted frames format. jpg or png allowed. Default is jpg')
parser.add_argument('--dest', type=str,
                    help='optional destination folder. Default is a new folder in the video path')
...
if not args.dest:
    ...
    dest = safe_mkdir(f'{args.dest}/{dest_folder_name}_extracted_frames')
...
def main(video_path=args.video_path, frame_name=frame_name, frame_format=frame_format, dest=dest):
    ...
    for _ in (t := trange(total)):
        ...
        if not success:
            break
        file = f'{dest}/{frame_name}_{str(count).zfill(6)}.{frame_format}'
